...
pos_files="func_tests/data/pos_??_args.txt"
for pos_file_in in $pos_files; do
	...
	if [ -f "$pos_file_args" ]; then
	  command="./func_tests/scripts/pos_case.sh ""$pos_file_out ""$pos_file_args"
	else
	  command="./func_tests/scripts/pos_case.sh ""$pos_file_out"
	fi
...
neg_files="func_tests/data/neg_??_args.txt"
for neg_file_in in $neg_files; do
  ...
  if [ -f "$neg_file_args" ]; then
  	  command="./func_tests/scripts/neg_case.sh ""$neg_file_args"
  else
